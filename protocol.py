...
#
#
# protocol.py copyright (C) 2002 Dustin G [email]
# original IRC library (irclib.py) opyright (C) 1999, 2000 Joel Rosdahl <joel@rosdahl.net>, edited 2002 Dustin G [email]
# 
...
# Dustin G [email]
#
# $Id: protocol.py,v 0.1.5.2 2002/07/23 01:16:31 dustin Exp dustin $

#***********************************************************************
# NOTICE: Many comments are now outdated and invalid! XXX
...
VERSION = 0, 3, 1 #original irclib version
DEBUG = 0
FDEBUG = 0
USE_FLOOD_PROTECTION=1 #unstable

class BoxError(Exception):
    """Represents an exception for any of the servers contained by Box."""
    pass


class Box:
    """[Updated to support any number of arbitrary protocols!]
    Class that handles one or several server connections.

    When a Box object has been instantiated, it can be used to create
    Connection objects that represent the individal server connections.
    The responsibility of the Box object is to provide an event-driven
    ...
    The methods of most interest for a client writer are server,
    ...
    to the nickname a_nickname, then processs all incoming requests
    forever.
    """
    ...
    def IRCServer(self):
        """Creates and returns a IRCServerConnection object."""

        c = IRCServerConnection(self)
        self.connections.append(c)
        return c

    def BnetServer(self):
        """Creates and returns a BnetServerConnection object.
        XXX INCOMPLETE
        """
        
        c = BnetServerConnection(self)
        self.connections.append(c)
        return c
    ...
    def process_data(self, sockets):
        ...
        See documentation for Box.__init__.
        """
        for s in sockets:
    ...
    def process_timeout(self):
        ...
        while self.delayed_commands:
            if t >= self.delayed_commands[0][0]:
                if FDEBUG: print ">> line 157: <<"
                if FDEBUG: print "apply(%s, '%s')"%(self.delayed_commands[0][1], self.delayed_commands[0][2])
...
class ServerConnectionError(BoxError):
    pass
...
class IRCServerConnection(Connection):
 ...
 dropping of the connection triggers the disconnect event.
    ...
    def __init__(self, irclibobj):
        ...
        self.f_limit=(6,2)
        self.f_recent=[] #time of last 6 sends, ordered oldest to most recent 
        for n in range(6):
            self.f_recent.append(time.time()-5)
    ...
    def connect(self, server, port, nickname, password=None, username=None,
        ...
        self.bot_channel=None #channel to join when connected
        self.server_nick=self.server
    ...
    #after connecting, the bot joins here
    def get_bot_channel(self):
        return self.bot_channel
    def set_bot_channel(self, newchan):
        self.bot_channel=newchan

    # server nickname - used in link to say which server the
    # link is referring to
    def get_server_nick(self):
        return self.server_nick
    def set_server_nick(self, new_sn):
        self.server_nick=new_sn
    ...
    def privmsg(self, target, text):
        ...
        Uses flood protected send_raw: fsend_raw XXX"""
        # Should limit len(text) here!
        if USE_FLOOD_PROTECTION:
            if FDEBUG: print ">> connection.privmsg('%s', '%s') called <<"%(target, text)
            if FDEBUG: print ">> connection.fsend_raw('%s') called <<"% ("PRIVMSG %s :%s" % (target, text))
            self.fsend_raw("PRIVMSG %s :%s" % (target, text))
        else:
    ...
    def privmsg_many(self, targets, text):
        ...
        # Should limit len(text) here!
        if USE_FLOOD_PROTECTION:
            self.fsend_raw("PRIVMSG %s :%s" % (string.join(targets, ","), text))
        else:
    ...
    def fsend_raw(self, string):
        if FDEBUG: print ">> fsend_raw('%s') called <<"%(string)
        if FDEBUG: print "\tself.f_recent[:] = \t%s"%(self.f_recent[:])
        if self.f_recent[0]+2>time.time(): #dont send it yet!
            if FDEBUG: print "\t> send text later <"
            if FDEBUG: print "\tself.f_recent[0] = \t%s"%(self.f_recent[0])
            if FDEBUG: print "\ttime.time() = \t%s\n"%(time.time())
            if FDEBUG: print "\t: self.f_recent[0]+2>time.time()"

            #How long until it can be sent?
            wait_time=self.f_recent[0]+2-time.time()
            exec_time=wait_time+time.time()
            if FDEBUG: print "\twait_time = \t%s"%(wait_time)
            if FDEBUG: print "\texec_time = \t%s"%(exec_time)

            # Cycle the recent list, putting the time the function is
            # scheduled to be sent at the end of the list.
            # No matter what happens, it will be the next string
            # sent, right? XXX
            del self.f_recent[0]
            if FDEBUG: print "\t: del self.f_recent[0]\n; append exec_time"
            self.f_recent.append(exec_time)
            if FDEBUG: print "\tself.f_recent[:] =\t%s"%(self.f_recent[:])
            
            # Send the string when the delay has expired.
            # No need to use fsend_raw() because it already
            # registerd it has send time in the recent list.

            if FDEBUG: print "\tsleeping for wait_time"
            time.sleep(wait_time)
            self.send_raw(string)
            #self.execute_delayed(wait_time, self.send_raw, (string))
            #self.execute_at(exec_time, self.send_raw, (string))

            # You don't need to know the wait_time... XXX
            return wait_time

        
        else: #ok, send it now
            if FDEBUG: print "\t> send text now <"
            # Cycle the recent list
            del self.f_recent[0]
            self.f_recent.append(time.time())
            if FDEBUG: print "\tself.f_recent[:] =\t%s"%(self.f_recent[:])

            #self.execute_delayed(0, self.send_raw, (string))#execute now
            self.send_raw(string)
    ...
    def fsend_raw2(self, string):
        """uses connection.execute_at(), does not work due to a bug in execute_at"""
        if FDEBUG: print ">> fsend_raw('%s') called <<"%(string)
        if FDEBUG: print "\tself.f_recent[:] = \t%s"%(self.f_recent[:])
        if self.f_recent[0]+2>time.time(): #dont send it yet!
            if FDEBUG: print "\t> send text later <"
            ...
            # sent, right? XXX
            del self.f_recent[0]
            if FDEBUG: print "\t: del self.f_recent[0]; append exec_time"
            self.f_recent.append(exec_time)
            if FDEBUG: print "\tself.f_recent[:] =\t%s"%(self.f_recent[:])
            ...
            # No need to use fsend_raw() because it already
            # registerd it has send time in the recent list.
            if FDEBUG: print "\tself.execute_at(%s, self.send_raw, (%s))"%(exec_time, string)
            self.execute_at(exec_time, self.send_raw, (string))
...
class BnetServerConnection(Connection):
    def __init__(self, irclibobj):
        Connection.__init__(self, irclibobj)
        self.connected = 0  # Not connected yet.

    def connect(self, server, port, nickname, password):
    ...
    def process_data(self):
        ...
        for line in lines:
            ...
            if line[0]=='<': #someone spoke
                command = 'pubmsg'
                sender = string.split(line[1:], '>', 1)[0]     #nick
                arguments = string.split(line[1:], '>', 1)[1][1:] #message
                if ' ' in sender: #private message?
                    sender = string.split(sender, ' ', 1)[1] #strip 'from 
                    command = 'privmsg'

            elif line[0]=='[' and line[-1]==']': #someone entered/left
                sender, arguments = string.split(line[1:-1], ' ', 1)
                if   arguments == 'enters': command = 'join'
                elif arguments == 'leaves': command = 'part'

            #will text spoke by the bot ever be recieved through the socket? no
            #elif line[0] in (string.lowercase + string.uppercase + #bot spoke
            #                 string.digits + string.punctuation):
            #    sender=self.nickname
            #    arguments=line

            else: #uh oh, Dustin didn't correctly reverse engineer the protocol...
                sender='ERROR'
                arguments='Bug in bnet text protocol class.  Recieved: %s' % line



            if command == 'pubmsg':
                if DEBUG:
                    print "command: %s, source: %s, arguments: %s" % (
                        command, sender, arguments)
                    self._handle_event(Event(command, sender, None, m))
                else:
                    if DEBUG:
                        print "command: %s, source: %s, target: %s, arguments: %s" % (
                                command, prefix, target, [m])
